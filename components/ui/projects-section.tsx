      ...
      <div ref={containerRef} className="space-y-24 lg:space-y-32">
            ...
            {companies.map((company, i) => {
              ...
              return (
                <div key={i} className="company-logo flex items-center gap-3 group">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/5 ring-1 ring-white/10 group-hover:bg-white/10 group-hover:ring-white/20">
        ...
        <div className="projects-grid grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
          {projects.map((project, index) => (
            <div
              key={index}
              className="project-card group relative flex flex-col overflow-hidden rounded-[2.5rem] border border-white/10 bg-white/[0.02] backdrop-blur-xl hover:bg-white/[0.05] hover:border-white/20"
            >
              {/* Image Container */}
              <div className="relative aspect-[16/10] w-full overflow-hidden">
                ...
                {/* Image Overlay */}
                <div className="absolute inset-0 bg-gradient-to-t from-black via-black/40 to-transparent opacity-80 group-hover:opacity-40" />

                {/* Impact Badge */}
                <div className="absolute top-6 right-6 z-10">
                  <div className="rounded-full border border-white/20 bg-black/40 px-4 py-2 backdrop-blur-md group-hover:bg-white/10 group-hover:border-white/30">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100">
              ...
              <div className="flex flex-col gap-6 p-8 lg:p-10">
                <div className="space-y-3">
                  <span className="text-[10px] font-light uppercase tracking-[0.15em] text-white/50 group-hover:text-white/80">
                    {project.category}
                  </span>
                  <h3 className="text-2xl lg:text-3xl font-light tracking-tight text-white">
                    {project.title}
                  </h3>
                  <p className="text-base font-light leading-relaxed text-white/60 group-hover:text-white/80">
                    {project.description}
                  </p>
                </div>
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  {project.tags.map((tag, tagIndex) => (
                    <span
                      key={tagIndex}
                      className="rounded-full border border-white/5 bg-white/[0.03] px-4 py-1.5 text-[10px] font-light tracking-tight text-white/60 backdrop-blur-sm group-hover:border-white/10 group-hover:text-white/90"
                    >
                      {tag}
                    </span>
                  ))}
        ...
        <div className="flex flex-col items-center text-center space-y-8">
          ...
          <Link
            href="/projects"
            className="group inline-flex items-center gap-4 rounded-full border border-white/10 bg-white/5 px-8 py-4 text-sm font-light tracking-tight text-white backdrop-blur-sm hover:bg-white hover:text-black hover:scale-105"
          >
